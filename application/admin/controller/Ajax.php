...
class Ajax extends Backend
{
    ...
    public function weigh()
    {
        ...
        //2020-11-25 17:05:07Xiaohe 增加 
        if($table == 'route_price'){
            $res_data = Db::name($table)->where('id',$changeid)->find();
            
            $order_school = Db::name('route_school')->where('routeid',$res_data['routeid'])->find();
            if($order_school){
                $this->error('修改失败，此路线已有订单ID:'.$order_school['id']);
            }
        }
